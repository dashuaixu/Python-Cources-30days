...
:speech_balloon:可添加WeChat：xumaimeng    :label:备注：挑战python，获取帮助。



![](30DaysOfPython_banner_section1.png)
...
接着找到Python的安装目录，找到其中的`bin`文件夹，里面有`python3.x`（版本号和你当初安装的一致），选择即可。（这里每个人的安装目录可能都不一样，按自己的情况来）
...
所谓的缩进其实就是空白字符，例如按一下空格键。在很多文档编辑中，我们使用缩进来增加文档的可读性。在Python中，缩进被设计成为代码的一部分，用来组织代码块。在其他多数编程语言中，常设计花括号`{}`来组织代码块。Python代码中一个常见的错误就是使用了错误的缩进。
